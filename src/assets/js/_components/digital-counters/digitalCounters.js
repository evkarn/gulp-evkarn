// IMPORT DIGITAL-COUNTERS
// import { digitalCounters } from './src/_utils/digital-counters/index.js';
// END. IMPORT DIGITAL-COUNTERS


	// INIT DIGITAL-COUNTERS
		/*
			1000 — скорость отработки анимации в мс
		*/

		// window.addEventListener('load', digitalCounters(1000));
	// END. INIT DIGITAL-COUNTERS
...
export function digitalCounters(timeDuration) {
	...
	function digitalCountersAnimate(elemDC) {
		...
		const duration = parseInt(elemDC.dataset.digitalCounter) ? parseInt(elemDC.dataset.digitalCounter) : timeDuration;
	...
	let observer = new IntersectionObserver((entries, observer) => {
		entries.forEach(entry => {
			if (entry.isIntersecting) {
				...
				const digitalCountersItems = targetElement.querySelectorAll('[data-digital-counter]');
